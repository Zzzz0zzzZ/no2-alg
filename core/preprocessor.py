        ...
        if original_strategy_id in test_case['strategies']:
            ...
            if (strategy.get('replaceable', True)):
                ...
                if init_strategy_id in new_test_case['strategies'] and new_test_case['strategies'][
                    ...
                    # 去除重复的替换选项
                    new_test_case['replacement_options'][init_strategy_id] = list(
                        set(new_test_case['replacement_options'][init_strategy_id]))
...
def analyze_filtered_result(result):
    ...
    print(
        f"带有军队信息的飞机类型数量: {aircraft_with_army} ({(aircraft_with_army / aircraft_count) * 100 if aircraft_count else 0:.1f}%)")
    print(
        f"带有军队信息的弹药类型数量: {ammunition_with_army} ({(ammunition_with_army / ammunition_count) * 100 if ammunition_count else 0:.1f}%)")
